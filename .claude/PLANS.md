...
This document describes the requirements for an execution plan ("Plan"), a design document that a coding agent can follow to deliver a working feature or system change. Treat the reader as a complete beginner to this repository: they have only the current working tree and the single Plan file you provide. There is no memory of prior plans and no external context.

Plans are stored in `.claude/plans/` with incrementing numbers (e.g. `11-feature-name.md`).
...
The agent executing your plan can list files, read files, search, run the project, and run tests. It does not know any prior context and cannot infer what you meant from earlier milestones. Repeat any assumption you rely on. Do not point to external blogs or docs; if knowledge is required, embed it in the plan itself in your own words. If an Plan builds upon a prior Plan and that file is checked in, incorporate it by reference. If it is not, you must include all relevant context from that plan.

## Formatting

Each Plan is a standalone Markdown (.md) file in `.claude/plans/`. Do not nest triple-backtick code fences inside; when you need to show commands, transcripts, diffs, or code, present them as indented blocks. Use indentation for clarity rather than code fences inside a Plan to avoid ambiguity. Use two newlines after every heading, use # and ## and so on, and correct syntax for ordered and unordered lists.
