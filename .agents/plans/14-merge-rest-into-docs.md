...
- Decision: Address review findings #1, #2, #4a, #4b in this PR; defer #3 and #4d; keep #4c.
  Rationale: #1 (validate `operationId` before it builds a path) and #2 (validate the fetched v6 spec, not just the transport, and fall back to embedded) both harden input that this PR newly turned from embedded into build-time-fetched — cheap and worth it. #4a (stale "byte-identical/C3"/SKILL.md comments) and #4b (avoidable `httpVerbs` global vs AGENTS.md rule 7) are correctness/cleanup. Deferred: #3 (`anyOf`/`oneOf`/`allOf` rendering as `any`) is a faithful, gracefully-degrading port of existing xk6-rest behavior — an enhancement, not a regression, so it belongs in a follow-up rather than scope-creeping this consolidation. #4d (manual YAML-alias following could overflow on a self-referential anchor) is unreachable: v6 is JSON and v5 is hand-authored/controlled, and #2's parse-validation runs first. Kept: #4c (endpoint H1 shows the version-prefixed id) is intentional — it gives an agent reading the raw `.md` file the API version inline, while the slug/title stay bare for navigation.
  Date/Author: 2026-06-19 / ankur22 + agent.


## Outcomes & Retrospective

(2026-06-19, iteration 1 complete.) All six milestones landed as bisectable commits on branch `feat/merge-rest-into-docs` (commit SHAs shifted once via an autosquash that folded a CI lint fix back into its origin commit, then a review pass added input-hardening — operationId validation, fetched-spec validation — and cleanup — httpVerbs function, stale comments). Against the Purpose: a bundle built by `cmd/prepare` now contains `markdown/cloud-rest-api/**` plus `cloud-rest-api` entries in `sections.json`; the end-to-end testscript proves `k6 x docs cloud-rest-api` lists v5/v6, `k6 x docs cloud-rest-api/v6/<op>` renders an endpoint, `k6 x docs search` finds it, and completion offers it. There is no `k6 x rest` command, the v6 spec is fetched at build time with the embedded copy as fallback, the v5 spec is a static embed, and `go mod tidy` confirms zero new dependencies. The `restdoc` generator is imported only by `cmd/prepare`, so its embedded specs stay out of the extension binary.

Remaining / out of scope for this iteration (recorded for follow-up): the k6-extension-registry change so provisioning advertises this module's REST content and the retirement of the xk6-rest repo (external coordination, user-owned); making `sync.sh` OpenAPI-aware for tighter v6 freshness; `--source` previews of the REST section; rendering `anyOf`/`oneOf`/`allOf` schemas as `a | b` instead of `any` (review finding #3); and a depth guard for self-referential YAML anchors (review finding #4d, currently unreachable since v6 is JSON and v5 is controlled). None block the feature.
